...
client.on("guildMemberAdd", async (member) => {
    if (client.config[member.guild.id].botAdd === false) return;
...
client.on("roleCreate", async (role) => {
    if (client.config[role.guild.id].roleCreate === false) return;
...
client.on("roleUpdate", async (oldRole, newRole) => {
    if (client.config[oldRole.guild.id].roleUpdate === false) return;
    ...
    fs.access(path, fs.F_OK, (err) => {
        if (err) {
            ...
            if (RoleUpdate.executor.id === client.user.id) return;
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
    if (client.config[oldMember.guild.id].roleUpdate === false) return;
...
client.on("webhookUpdate", async channel => {
    if (client.config[channel.guild.id].webhookCreate === false) return;
...
// Webhook Delete
client.on("webhookUpdate", async channel => {
    if (client.config[channel.guild.id].webhookDelete === false) return;
    const eventsTimestamp = Date.now().toString();
...
client.on("clickButton", async (Button) => {
    ...
    function helpList (arr, button) {
        let arr2 = [];
        arr.forEach(element => {
            arr2.push(`**${ element.name }** - ${ element.description }\n\n`);
        });
        const embed = new MessageEmbed().setImage("https://cdn.discordapp.com/attachments/845386891993677837/947872027673518080/banner.png").setTitle("Help Menu").setColor(0x36393E).setThumbnail(`${ client.user.avatarURL({ dynamic: true }) }`).setDescription(`Commands List\n\n${ (arr2 + ``).replace(/,/g, "") }`).setTimestamp(Date.now()).setFooter(`© ${ client.user.username } | Prefix: ${ prefix } | By: ${ author }`);
        const backButton2 = new disbut.MessageButton().setStyle("gray").setLabel("Back").setID("back").setEmoji("⏪");
        let buttons2 = [backButton2];
        button.message.edit(embed, { buttons: buttons2 });
    }

    if (Button.id === "more" || Button.id === "back2") {
        const embed = new MessageEmbed().setImage("https://cdn.discordapp.com/attachments/845386891993677837/947872027673518080/banner.png").setTitle("Help Menu").setColor(0x36393E).setTimestamp(Date.now()).setFooter(`© ${ client.user.username } | Prefix: ${ prefix } | By: ${ author }`).setThumbnail(`${ client.user.avatarURL({ dynamic: true }) }`).setDescription(`Please choose help list you need
                      
        **Make sure bot's role is on top**
`);
        const backButton = new disbut.MessageButton().setStyle("gray").setLabel("Back").setID("back").setEmoji("◀️");
        const setupButton = new disbut.MessageButton().setStyle("green").setLabel("Setup").setID("setup").setEmoji("💡");
        const supportButton = new disbut.MessageButton().setStyle("green").setLabel("Support").setID("support").setEmoji("🤝");
        const musicButton = new disbut.MessageButton().setStyle("green").setLabel("Music").setID("music").setEmoji("🎵");
        const moreButton2 = new disbut.MessageButton().setStyle("gray").setLabel("More").setID("more2").setEmoji("▶️");
        let buttons = [backButton, setupButton, musicButton, supportButton, moreButton2];
        await Button.message.edit(embed, { buttons: buttons });
    }
    ...
    if (Button.id === "more2") {
        ...
        **Make sure bot's role is on top**
`);
        const backButton = new disbut.MessageButton().setStyle("gray").setLabel("Back").setID("back2").setEmoji("◀️");
        const otherButton = new disbut.MessageButton().setStyle("green").setLabel("Other").setID("other").setEmoji("🛠️");
        let buttons = [backButton, otherButton];
        await Button.message.edit(embed, { buttons: buttons });
    }

    if (Button.id === "blacklist") {
        helpList(blacklist, Button);
    }

    if (Button.id === "whitelist") {
        helpList(whitelist, Button);
    }

    if (Button.id === "trust") {
        helpList(trust, Button);
    }

    if (Button.id === "config") {
        helpList(config, Button);
    }

    if (Button.id === "setup") {
        helpList(setup, Button);
    }

    if (Button.id === "support") {
        const embed = new MessageEmbed().setImage("https://cdn.discordapp.com/attachments/845386891993677837/947872027673518080/banner.png").setTitle("Help Menu").setColor(0x36393E).setThumbnail(`${ client.user.avatarURL({ dynamic: true }) }`).setTimestamp(Date.now()).setFooter(`© ${ client.user.username } | Prefix: ${ prefix } | By: ${ author }`).setDescription(`
Get support: DM MX1D#7609

Bot invite: https://discord.com/api/oauth2/authorize?client_id=934804583513989192&permissions=8&scope=bot%20applications.commands
        `);
...
**youtube** - Starts a YouTube Together session
        `);
        const backButton3 = new disbut.MessageButton().setStyle("gray").setLabel("Back").setID("back").setEmoji("⏪");
        let buttons3 = [backButton3];

        Button.message.edit(embed, { buttons: buttons3 });
    }

    if (Button.id === "other") {
        helpList(other, Button);
    }
    ...
    if (Button.id === "back") {
        ...
        let buttons2 = [blacklistButton, whitelistButton, trustButton, configButton, moreButton];
        const help = new MessageEmbed().setTitle("Help Menu").setThumbnail(`${ client.user.avatarURL({ dynamic: true }) }`)
...
`)
        ...
        Button.message.edit(help, { buttons: buttons2 });
    }
